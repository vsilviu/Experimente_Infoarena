        ...
        implements Comparable<DecimalBigInt>
{
    ...
    public static int countOfZeroesInFactorial(int n) {
        DecimalBigInt fac = factorial(n);
        int zeroCount = 0;
        for(int i = fac.digits.length - 1; i >= 0; --i) {
            int crtElem = fac.digits[i];
            if(crtElem == 0) zeroCount += 9;
            else {
                while (crtElem %10 == 0) {
                    ++zeroCount;
                    crtElem /= 10;
